...
class LaunchConfigPage{
  ...
  constructor(page: Page) {
    ...
    this.fillValidation = page.getByLabel('Validation');
    this.clearText = page.locator('//input[@id="#/properties/configmgr/properties/validation4"]/following-sibling::div//button[@aria-label="Clear"]')
    ...
    this.cachingService_title = page.locator('//div[text()="CachingService"]')
  ...
  async fillvalues(validation:string){
    ...
    await inputLocator.fill(validation);
 ...
 async get_logLevel_value(){
  ...
  async click_skipLaunhConfig(){
   ...
   const reviewPage_title = await this.reviewPage_title.textContent();
   return reviewPage_title;
  }
  ...
  async returnTitleOfPrevPage(){
   ...
   const caching_service_title = await this.cachingService_title.textContent();
   return caching_service_title;
  }
